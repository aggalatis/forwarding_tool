...
DbClass.prototype.getServerDataFromRegistry = function () {
    ...
    var dbFile = fs.readFileSync("/home/aggalatis/Projects/forwarding_tool/dbdata.agcfg", "utf8")
...
DbClass.prototype.confirmPersonnel = function (personnelID) {
    ...
    var sql = "UPDATE individuals SET " + 'ind_status = "Done", ' + 'ind_confirmation_date = "' + self.Helpers.getDateTimeNow() + '"' + " WHERE ind_id = " + personnelID + " AND ind_deleted = 0;"
...
DbClass.prototype.assignJobsToNewGroup = async function () {
    ...
    let conAdded = await new Promise(function (resolve, reject) {
        let sql = ""
        for (let ind of inds) {
            sql = `INSERT INTO consolidations (con_id, con_user_id, con_division_id, con_products, con_vessels, con_reference, con_kg, con_status, con_request_date, con_group_id)
...
DbClass.prototype.initializeManagerTable = function (indData, conData) {
    ...
    var pieData = {
        ...
        labels: ["Individual (" + total_individual_cost_per.toFixed(2) + "%)", "Consolidation (" + total_con_cost_per.toFixed(2) + "%)", "Grouped (" + total_grouped_cost_per.toFixed(2) + "%)"],
    }
...
DbClass.prototype.confirmConsGroup = async function (data) {
    ...
    let conAdded = await new Promise(function (resolve, reject) {
        let sql = ""
        for (let con of cons) {
            sql = `INSERT INTO consolidations_done (cond_id, cond_user_id, cond_division_id, cond_products, cond_vessels, cond_reference, cond_kg, cond_status, cond_request_date, cond_group_id)
                    VALUES (${con.con_id}, ${self.Helpers.user_id}, ${con.con_division_id}, '${con.con_products}', '${con.con_vessels}', '${con.con_reference}', ${con.con_kg}, 'Done', '${con.con_request_date}', ${con.con_group_id});`
            connection.query(sql, (err, data) => {
                if (err) {
