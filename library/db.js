...
DbClass.prototype.initializeGroupCitySelect = function (
  exCityName,
  toCityName
) {
  let self = this;
  ...
  for (i = 0; i < self.cities.length; i++) {
    ...
    $("#group-ex-select").append(
      new Option(self.cities[i].city_name, self.cities[i].city_id)
    );
    $("#group-to-select").append(
      new Option(self.cities[i].city_name, self.cities[i].city_id)
    );
  }
...
DbClass.prototype.getAllIndividuals = function () {
  ...
  connection.query(sql, function (error, data) {
    ...
    $("#jobs_table").on("click", "i.job-edit", function () {
      ...
      self.Helpers.initliazeModalToEditJob(
        self.divisions,
        self.products,
        self.vessels,
        self.cities,
      ...
      $("#save-job-btn").on("click", function () {
        ...
        var estimatecostSelectValue = self.Helpers.formatFloatValue(
          $("#estimate_cost").val()
        );
        ...
        if (
          ...
          for (i = 0; i < self.products.length; i++) {
            if (
              productSelectValue.indexOf(
                self.products[i].product_id.toString()
              ) !== -1
            ) {
          ...
          for (i = 0; i < self.vessels.length; i++) {
            if (
              vesselSelectValue.indexOf(
                self.vessels[i].vessel_id.toString()
              ) !== -1
            ) {
    ...
    $("#jobs_table").on("click", "i.costs-job", function () {
      ...
      if (data[5] == "Personnel") {
        ...
        $("#personnel-modal-header").removeClass(
          "noFloat floatMeLeft floatMeRight"
        );
        ...
        if (data[16] == 0) {
          ...
          var savings_percent = (
            (1 - group_cost / sum_estimate_cost) *
            100
          ).toFixed(2);
          var savings_amount = ((data[13] * savings_percent) / 100).toFixed(2);
          var shared_cost = (
            (group_cost / sum_estimate_cost) *
            data[13]
          ).toFixed(2);
          ...
          if (data[18] == null) {
            ...
            $("#group_cost").val(
              self.Helpers.formatFloatValue(String(group_cost))
            );
          }
...
DbClass.prototype.getAllDoneIndividuals = function () {
  ...
  connection.query(sql, function (error, data) {
    ...
    $("#done_individuals_table").on("click", "i.done-job-cost", function () {
      $("#cost-data-modal-header").removeClass(
        "noFloat floatMeLeft floatMeRight"
      );
      $("#done-personnel-modal-header").removeClass(
        "noFloat floatMeLeft floatMeRight"
      );
      var data = jobs_table.row($(this).closest("tr")).data();
      ...
      if (data[4] != "Grouped") {
        ...
        var shared_cost = ((group_cost / sum_estimate_cost) * data[16]).toFixed(
          2
        );
...
DbClass.prototype.deleteIndividual = function (individualData) {
  ...
  var sql =
    "UPDATE individuals set ind_deleted = 1, ind_date_deleted = now() WHERE ind_id = " +
    individualData[0] +
    ";";
...
DbClass.prototype.handleIndividualGroups = function (
  individualData,
  insertedID
) {
  let self = this;
      ...
      if (groupResults.length == 0) {
        ...
        if (groupResults[0].ind_group_color == "empty") {
          var randomNumber = Math.floor(
            Math.random() * (self.colors.length - 1)
          );
...
DbClass.prototype.handleGroupForPersonnel = function (individualId) {
  ...
  var updateInd =
    "UPDATE individuals SET ind_group_id = 0, ind_is_grouped = 0 WHERE ind_id = " +
    individualId +
    ";";
...
DbClass.prototype.updatePersonnelCosts = function (
  personnelId,
  personnelActualCost
) {
  let self = this;

  var updateActualCost =
    "UPDATE individuals SET ind_actual_cost = " +
    personnelActualCost +
    " WHERE ind_id = " +
    personnelId +
    ";";
...
DbClass.prototype.addVessel = function (vesselName) {
  ...
  var sql =
    'INSERT INTO vessels (vessel_description) VALUES ("' + vesselName + '");';
...
DbClass.prototype.initializeManagerTable = function (indData, conData) {
  ...
  for (i = 0; i < self.divisions.length; i++) {
    ...
    for (j = 0; j < indData.length; j++) {
      if (indData[j].division_id == self.divisions[i].division_id) {
        if (indData[j].ind_is_grouped == 0) {
          ...
          division_estimate_cost =
            division_estimate_cost + indData[j].ind_estimate_cost;
          ...
          var savings_amount =
            (parseFloat(indData[j].ind_estimate_cost) * savings_percent) / 100;
          var shared_cost =
            (group_cost / sum_estimate_cost) *
            parseFloat(indData[j].ind_estimate_cost);
    ...
    for (j = 0; j < conData.length; j++) {
      if (conData[j].division_id == self.divisions[i].division_id) {
        var sum_chargable_weight = parseFloat(
          conData[j].consolidation_group_weight
        );
        var chargable_weight = parseFloat(conData[j].con_chargable_weight);

        var jobCost =
          (chargable_weight / sum_chargable_weight) *
          parseFloat(conData[j].con_group_cost);
    ...
    if (division_shared_cost == 0 || division_estimate_cost == 0) {
      ...
      var finalSavingPercent = (
        100 -
        (division_shared_cost / division_estimate_cost) * 100
      ).toFixed(2);
    }
    ...
    dataset[i].push(
      (division_ind_cost + division_con_cost + division_shared_cost).toFixed(
        2
      ) + " €"
    );
    total_estimate_cost =
      parseFloat(total_estimate_cost) +
      division_ind_cost +
      division_con_cost +
      division_estimate_cost;
    total_shared_cost =
      parseFloat(total_shared_cost) +
      division_con_cost +
      division_ind_cost +
      division_shared_cost;

    total_individual_cost =
      parseFloat(total_individual_cost) + division_ind_cost;
  ...
  dataset[9].push(
    "<b>" +
      (
        ((estimateCostSum - total_grouped_cost) / estimateCostSum) *
        100
      ).toFixed(2) +
      " %" +
      "</b>"
  ...
  dataset[9].push(
    "<b>" + (estimateCostSum - total_grouped_cost).toFixed(2) + " €" + "</b>"
  );
  ...
  var total_individual_cost_per =
    (total_individual_cost /
      (total_individual_cost + total_con_cost + total_grouped_cost)) *
    100;
  var total_con_cost_per =
    (total_con_cost /
      (total_individual_cost + total_con_cost + total_grouped_cost)) *
    100;
  var total_grouped_cost_per =
    (total_grouped_cost /
      (total_individual_cost + total_con_cost + total_grouped_cost)) *
    100;
  ...
  var pieData = {
    datasets: [
      {
        data: [
          total_individual_cost.toFixed(2),
          total_con_cost.toFixed(2),
          total_grouped_cost.toFixed(2),
        ],
