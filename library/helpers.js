let HelpersClass = function () {
  ...
  console.log(
    "%c App Created by AG! Visit: aggalatis.com",
...
HelpersClass.prototype.initializeHelp = function () {
  ...
  $("#help-instructions").on("click", function () {
    let instructions = self.instructionFiles.find(
      (el) => el.type === "instructions"
    );
    window.open(instructions.url);
  });

  $("#help-visualizer").on("click", function () {
    let visualizer = self.instructionFiles.find(
      (el) => el.type === "visualizer"
    );
...
HelpersClass.prototype.initliazeModalToEditJob = function (
  divisions,
  products,
  vessels,
  cities,
  ...
  self.findChoosenValueForServiceType(
    serviceTypes,
    jobData.service_type_description
  );
...
HelpersClass.prototype.findChoosenValueForDivision = function (
  divisions,
...
HelpersClass.prototype.findChoosenValueForServiceType = function (
  serviceTypes,
...
HelpersClass.prototype.findChoosenValueForProducts = function (
  products,
  product_names
) {
  let self = this;
  ...
  for (i = 0; i < products.length; i++) {
    if (
      productNames.indexOf(products[i].product_description.toString()) !== -1
    ) {
...
HelpersClass.prototype.findChoosenValueForVessels = function (
  vessels,
  vessels_description
) {
  let self = this;
...
HelpersClass.prototype.findChoosenValueForCities = function (
  cities,
  ex_city,
  to_city
) {
